...
/* ---- 5. label points ------------------------------------------------------
   The hand-entered label_point values in units.json were eyeballed. A centroid
   is not a safe replacement: Kerala is a thin coastal arc and Maharashtra wraps
   around, so an area centroid can land in the sea or in a neighbour. This finds
   the pole of inaccessibility instead — the interior point furthest from any
   boundary — by grid search refined around the best cell, which is plenty for
   placing a label and needs no dependency. */
const featsForLabels = toFeatures(pick.t);

function poleOfInaccessibility(geom) {
  const polys = geom.type === "Polygon" ? [geom.coordinates] : geom.coordinates;
  /* Label the largest ring by bounding-box area; for a multipolygon that is the
     mainland rather than an outlying island. */
  ...
  const ring = best[0], holes = best.slice(1);
  const xs = ring.map(c => c[0]), ys = ring.map(c => c[1]);
  const [x0, x1, y0, y1] = [Math.min(...xs), Math.max(...xs), Math.min(...ys), Math.max(...ys)];

  const inside = (x, y) =>
    ringHas(ring, x, y) && !holes.some(h => ringHas(h, x, y));
  const distToRings = (x, y) => {
    let m = Infinity;
    for (const r of best) {
      for (let i = 0, j = r.length - 1; i < r.length; j = i++) {
  ...
  let cx = (x0 + x1) / 2, cy = (y0 + y1) / 2, cd = -1;
  let step = Math.max((x1 - x0), (y1 - y0)) / 24;
  for (let pass = 0; pass < 4; pass++) {
    const lo = [cd < 0 ? x0 : cx - step * 3, cd < 0 ? y0 : cy - step * 3];
    const hi = [cd < 0 ? x1 : cx + step * 3, cd < 0 ? y1 : cy + step * 3];
    for (let x = lo[0]; x <= hi[0]; x += step) {
      for (let y = lo[1]; y <= hi[1]; y += step) {
        if (!inside(x, y)) continue;
        const d = distToRings(x, y);
        if (d > cd) { cd = d; cx = x; cy = y; }
      }
    }
    step /= 3;
  }
  return { lon: Number(cx.toFixed(4)), lat: Number(cy.toFixed(4)), clearance: Number(cd.toFixed(4)) };
}

const labelPoints = {};
for (const f of featsForLabels) labelPoints[f.properties.code] = poleOfInaccessibility(f.geometry);
writeFileSync(join(ROOT, "data", "label-points.json"), JSON.stringify({
  $comment: "Generated by scripts/build_boundaries.mjs. Pole of inaccessibility per unit, " +
            "for label placement only. Never use these as geometry.",
  points: labelPoints,
}, null, 2) + "\n");
console.log(`\ndata/label-points.json  ${Object.keys(labelPoints).length} interior points computed`);

/* ---- 6. the check image --------------------------------------------------- */
const feats = featsForLabels;
